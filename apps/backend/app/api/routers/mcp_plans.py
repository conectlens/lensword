...
async def execute(
    ...
    for step in stored.plan.steps:
        try:
            result = await invoke(InvokeRequest(tool=step.tool, requester=stored.request.requester, workspace=stored.request.workspace, payload=step.payload), current_user, db, groups, words, sessions, exercises, provider, companion_sessions, companion_tasks, recall_settings)
