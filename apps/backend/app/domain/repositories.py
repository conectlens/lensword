...
from app.domain.services.companion_loop import CompanionLoopState
from app.domain.services.companion_sampling_audit import CompanionSamplingEvent
...
class DiagnosisRepository(Protocol):
    ...
    # Account-wide, across every word — issue #192's `/me/diagnoses`
    # companion resource, which has no single word to scope to. Newest
    # first, the same ordering `list_for_word` already uses.
...
class InterventionRepository(Protocol):
    ...
    # `limit`/`offset` default to no bound at all, matching every existing
    # caller (`review.py`'s contrast-card wiring wants every plan). Issue
    # #192's `/me/interventions` companion resource is the first caller to
    # pass them, for real pagination rather than loading an unbounded
    # account history into one response.
...
class CompanionSessionRepository(Protocol):
    ...
    def update(self, session: CompanionSession, *, expected_revision: int) -> CompanionSession:
        """Persist `session`, but only if its stored revision is still
        `expected_revision`. Raises ConcurrentModificationError otherwise
        (see app.domain.exceptions) — true optimistic locking, not a
        blind overwrite."""
        ...
...
class CompanionLoopStateRepository(Protocol):
    """One durable loop-budget row per session (#195 TODO 2). `start`
    replaces any prior state for the session — a fresh workflow gets a
    fresh budget rather than inheriting a previous, possibly-stopped one."""
...
class CompanionSamplingEventRepository(Protocol):
    """Append-only, hash-chained provenance records (#195 TODO 4)."""
