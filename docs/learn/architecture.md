---
title: Architecture
description: Backend and frontend architecture, and the design decisions behind them.
---
...
- **AI mnemonic suggestions are real, opt-in, and local.** MnemoLab can ask a
  ...
  suggestions](../install/local-ai-ollama.md). Image generation is still not implemented —
...
- **Reminders reach the desktop; push and email still only reach the log.**
  ...
  app is remote-only ([ADR 0002](../reference/adr/0002-desktop-backend-mode.md)), the
