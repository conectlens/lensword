...
|---|---|---|---|
| 2026-08-09 | Desktop Application | CI-only change. It repairs the Continuous Desktop Build workflow's own artifact upload step and touches no application code, so no released desktop build behaves differently as a result. The installers this workflow produces were already correct — they were simply not being attached to the run, which is a property of the pipeline rather than of the product. | [PR #346](https://github.com/conectlens/lensword/pull/346) |
