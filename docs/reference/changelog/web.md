...
- This is the first reversibly-encrypted secret this codebase has ever stored (every prior credential is one-way hashed) and handles real financial-risk credentials — flagged as needing a human security review before production use, not self-certified safe by tests passing.
