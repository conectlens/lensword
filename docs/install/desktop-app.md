...
- **Endpoint resolution order:** `LENSWORD_API_URL` env var, then an `api-endpoint` file in the OS application-config directory, then `http://127.0.0.1:8000`. Must be loopback or `https://` — plain HTTP to a remote host is refused at the Rust layer, not just discouraged.
...
> **No release has been published yet** — confirmed via `git tag -l` and
> `gh release list`, both empty. Until one exists, every platform means
> building from source below. When a release does exist, its artifacts are
